...
begin
  if exists (
    ...
    where table_schema = 'public'
      ...
      and column_name = 'modo_oscuro'
  ) then
    execute $sql$
      with principal_empresa as (
        select distinct on (ue.usuario_id)
          ue.usuario_id,
          ue.empresa_id
        from public.usuario_empresas ue
        order by ue.usuario_id, ue.es_principal desc, ue.created_at asc
      )
      update public.usuarios u
      set modo_oscuro = e.modo_oscuro
      from principal_empresa pe
      join public.empresas e on e.id = pe.empresa_id
...
alter table public.empresas
  drop column if exists modo_oscuro;
